...
logger = logging.getLogger("ads_ai.gads.step12")
...
def _select_panel(driver: WebDriver, panel_syns: Sequence[str]) -> None:
    ...
    logger.info("step12: ensure_panel_open(panel=%s) -> %s", list(panel_syns), ok)
    if not ok:
...
def _dedupe_files(paths: Iterable[str]) -> List[str]:
    ...
    for raw in paths:
        ...
        out.append(norm)
    if dropped:
        logger.warning("step12: удалены дубли перед загрузкой: %s", list(dict.fromkeys(dropped))[:8])
    return out
...
def _maybe_shot(driver: WebDriver, path: Path, label: str) -> None:
    ...
    try:
        ...
        logger.info("step12: screenshot saved [%s] -> %s", label, path)
    except Exception as exc:
        logger.debug("step12: screenshot failed [%s]: %s", label, exc)
...
class UploadDialog:
    ...
    def open(self) -> None:
        logger.info("step12: [%s] открытие диалога — ensure panel + поиск кнопки Add", self.kind)
        ...
        logger.info("step12: overlay panes before click: %d", before)
        ...
        for i in range(48):
            ...
            logger.debug("step12: wait modal (%s) tick=%d | overlays %d→%d | dlg=%s", self.kind, i, before, now, bool(dlg))
        ...
        logger.info("step12: открыт диалог %s (overlays=%d)", self.kind, self._overlay_count())
    ...
    def ensure_upload_tab(self) -> bool:
        for attempt in range(1, 4):
            ...
            logger.info("step12: [%s] попытка активировать вкладку Upload #%d -> %s", self.kind, attempt, bool(tab))
            if not tab:
    ...
    def _click_upload_from_computer(self) -> bool:
        ...
        try:
            ...
            if not btn:
                logger.info("step12: [%s] кнопка 'Upload from computer' не найдена", self.kind)
                return False
    ...
    def locate_input(self, *, try_click_upload_button: bool = True) -> Optional[WebElement]:
        ...
        logger.info("step12: [%s] поиск input[type=file] первичный -> %s", self.kind, bool(el))
        if el:
            return el
        if try_click_upload_button:
            ...
            for i in range(35):
                el = self._ensure_file_input_visible(self.root)
                logger.debug("step12: [%s] re-check input[type=file] #%d -> %s", self.kind, i + 1, bool(el))
                if el:
                    return el
                time.sleep(0.15)
        return None
    ...
    def wait_save_and_click(self, *, min_wait: float = 3.0, timeout: float = 120.0) -> bool:
        ...
        logger.info("step12: жду активации Save/Select/Done (мин. ожидание %.1fs, таймаут %.1fs)", min_wait, timeout)
        while time.time() < dl:
            btn = self._locate_confirm_button(enabled_only=True)
            if not btn:
                probe = self._locate_confirm_button(enabled_only=False)
                if probe:
                    logger.info("step12: confirm-button присутствует, но дизейблен — ждём…")
            else:
                if time.time() >= earliest and self._button_is_enabled(btn):
                    ...
                    if _mouse_click(self.drv, btn):
                        ...
                        logger.info("step12: клик по Save/Select/Done -> OK")
                        return True
            time.sleep(0.25)
        return False
    ...
    def wait_closed(self, *, timeout: float = 15.0) -> bool:
        ...
        while time.time() < dl:
            try:
                ...
                if not still:
                    logger.info("step12: диалог закрыт (%s)", self.kind)
                    return True
            except Exception:
                return True
            time.sleep(0.2)
        logger.info("step12: диалог (%s) не закрылся вовремя", self.kind)
        return False
...
class Uploader:
    ...
    def _ack_wait(self, start_count: int, filename: Optional[str], *, is_first: bool, t_first: float = 24.0, t_next: float = 10.0) -> Tuple[bool, Dict[str, Any]]:
        ...
        while time.time() < dl:
            ...
            if errors:
                logger.warning("step12: ошибки диалога: %s", "; ".join(errors[:3]))
    ...
    def _log_input_snapshot(self, input_el: Optional[WebElement], *, label: str) -> None:
        ...
        try:
            ...
            pass
        logger.info("step12: [%s] input snapshot = %s", label, info)
    ...
    def attach(self, files: List[str], *, prefer_cdp: bool = True) -> None:
        ...
        logger.info("step12: старт attach (%s), базовый previews=%d, известные имена=%s",
        ...
        if not queue:
            logger.info("step12: все кандидаты уже присутствуют в диалоге — ничего крепить не нужно")
            return
    ...
    def _attach_batch(self, input_el: WebElement, token: str, batch: List[str], *, prefer_cdp: bool, base_count: int) -> None:
        ...
        try:
            ...
            logger.debug("step12: batch attach failed (%s)", exc)
        ...
        if not accepted:
            logger.warning("step12: batch via=%s не дал видимого ack — последовательный режим", via)
            for i, p in enumerate(batch, 1):
                self._attach_one(p, input_el, token, is_first=(i == 1 and base_count == 0), prefer_cdp=prefer_cdp)
                self.dlg.ensure_upload_tab()
                input_el = self.dlg.locate_input() or self._refresh_latest_input(input_el, token)
        else:
            logger.info("step12: batch via=%s принят модалкой", via)
    ...
    def _attach_one(self, path: str, input_el: WebElement, token: str, *, is_first: bool, prefer_cdp: bool) -> None:
        ...
        if base_name in self.attempted or base_name in self.known_names:
            logger.info("step12: SKIP '%s' (уже попытан или известен в диалоге)", base_name)
            return
        ...
        logger.info("step12: attach via=%s ok=%s", attached_via, ok)
        ...
        if not accepted and files_len_after > 0:
            logger.info("step12: dialog ack=false, но input.files.length=%s — считаю принятым", files_len_after)
            accepted = True
            ...
            if not accepted:
                if base_name in _extract_names_from_snap(snap):
                    ...
                    if not accepted:
                        ...
                        if errs:
                            logger.warning("step12: SKIP '%s' — отклонено UI: %s", base_name, "; ".join(errs[:3]))
                            return
                        logger.warning("step12: SKIP '%s' — UI не показал приёмку (via=%s, send_exc=%s)", base_name, attached_via, send_exc)
                        return
                ...
                if btn:
                    ...
                    for _ in range(20):
                        if not _detect_editor(self.dlg.snapshot()):
                            logger.info("step12: редактор закрыт (Done/Save)")
                            return
                        time.sleep(0.2)
            else:
                if was_editor:
...
def _llm_generate_image_prompts(*, count: int, business_name: Optional[str], usp: Optional[str], site_url: Optional[str], mode: str, seed_notes: Optional[str]) -> List[str]:
    if STEP12_DISABLE_LLM or GeminiClient is None:
        logger.warning("step12: LLM отключена — fallback промпты для изображений")
    ...
    try:
        ...
        logger.warning("step12: LLM промпты (images) не получены: %s", exc)
        return _fallback_image_prompts(count=count, business_name=business_name, usp=usp, mode=mode)
...
def _llm_generate_logo_prompts(*, count: int, business_name: Optional[str], usp: Optional[str], seed_notes: Optional[str]) -> List[str]:
    ...
    try:
        ...
        logger.warning("step12: LLM промпты (logos) не получены: %s", exc)
        return _fallback_logo_prompts(count=count, business_name=business_name, usp=usp)
...
def _generate_images_via_runware(*, prompts: List[str], dest_dir: Path, label_prefix: str, width: int, height: int) -> List[str]:
    ...
    for i, p in enumerate(prompts, 1):
        try:
            ...
            logger.error("step12: Runware картинка #%d не сгенерирована: %s", i, exc)
    return out
...
def run_step12(
    ...
    *,
    ...
    # ---------- Изображения ----------
    if normalized_mode in {"ai_only", "inspired"}:
        _emit(emit, "Генерирую промпты для изображений")
        cnt = max(1, desired_image_count)
        image_prompts = _llm_generate_image_prompts(count=cnt, business_name=business_name, usp=usp, site_url=site_url, mode=normalized_mode, seed_notes=seed_notes)
        generation_note = "Промпты сгенерированы автоматически." if not seed_notes else "Промпты сгенерированы на основе ваших примеров."
        tm.mark("generate_image_prompts")

        _emit(emit, f"Генерирую изображения через Runware ({len(image_prompts)} шт.)")
        images_to_upload = _generate_images_via_runware(
            prompts=image_prompts, dest_dir=Path(storage_dir), label_prefix="demandgen-image",
            width=RUNWARE_IMAGE_WIDTH, height=RUNWARE_IMAGE_HEIGHT,
        )
        tm.mark("generate_images_runware")

        if len(images_to_upload) < cnt and isinstance(seed_assets, dict):
            fallback_imgs = _collect_manual_assets(seed_assets.get("images"), dest_dir=Path(storage_dir))
            images_to_upload.extend(fallback_imgs[: cnt - len(images_to_upload)])
    else:
        _emit(emit, "Использую предоставленные изображения (manual)")
        images_to_upload = manual_images[: max(1, desired_image_count)] if manual_images else []
        if not images_to_upload:
            raise Step12Error("В режиме manual требуется хотя бы одно изображение.")
    ...
    with _ConfirmWatcher(driver, emit=emit):
        # Images
        _emit(emit, "Открываю диалог изображений")
        dlg_images = UploadDialog(driver, "images", storage_dir=Path(storage_dir))
        dlg_images.open()
        if not dlg_images.ensure_upload_tab():
            logger.warning("step12: вкладка Upload (images) не активировалась — tolerant режим")
        time.sleep(0.2)

        _maybe_shot(driver, Path(storage_dir) / "debug" / "before_attach_images.png", "before-attach-images")

        uploader_images = Uploader(driver, dlg_images, kind="images")
        uploader_images.attach(images_to_upload, prefer_cdp=True)

        if not dlg_images.wait_save_and_click(min_wait=5.0, timeout=180.0):
            snap = dlg_images.snapshot()
